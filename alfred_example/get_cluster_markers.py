...
'''
marker_dict=dict([line.strip().split() for line in open('cell_type_labels.tsv')])
#print(marker_dict)
import custom2
cluster_dict={}
adjusted_p_c, cluster_c, gene_c=5,6,7
for line_number, line in enumerate(open('markers.tsv')):
	if line_number>0:
...
output_file=open('actual_cluster_labels.txt', 'w')
